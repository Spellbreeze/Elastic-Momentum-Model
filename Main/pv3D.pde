...
class VCT 
   ...
   VCT rotate(float a, VCT U) {setTo(R(this,a,U)); return this;} // rotate this V by a around U, assuming|U|==1 and V*U==0
...
VCT Vlerp(VCT A, float s, VCT B) {return new VCT(A.x+s*(B.x-A.x),A.y+s*(B.y-A.y),A.z+s*(B.z-A.z)); };      // (1-s)A+sB
...
VCT V(VCT U, float s, VCT V) {return V(U.x+s*V.x,U.y+s*V.y,U.z+s*V.z);};                               // U+sV
...
VCT CrossProduct(VCT U, VCT V) {return V( U.y*V.z-U.z*V.y, U.z*V.x-U.x*V.z, U.x*V.y-U.y*V.x); };                  // UxV cross product (normal to both)
...
float mixed(VCT U, VCT V, VCT W) {return dot(U,CrossProduct(V,W));}
...
float mixedProduct(VCT U, VCT V, VCT W) {return d(U,N(V,W)); };                                                 // U*(VxW)  mixed product, determinant
...
VCT R(VCT V, float a, VCT U) {VCT W=CrossProduct(U,V); return V(cos(a),V,sin(a),W);} // rotate V by a around U, assuming|U|==1 and V*U==0
VCT LPM(VCT U, float t, VCT V) // log-polar morph between U and V
  {
  VCT B=U(CrossProduct(U,V)); 
  VCT N=CrossProduct(B,U); 
  float a = angle(U,V);
  VCT W = V(cos(a*t),U,sin(a*t),N);
  float m = n(V)/n(U);
  return V(pow(m,t),W);
  } 
...
void showSquashedBall(PNT P, float r, float s) {pushMatrix(); translate(P.x,P.y,P.z); scale(1,1,s); sphere(r); popMatrix();};                          // render sphere of radius r and center P
...
//===== SUBDIVISION
PNT B(PNT A, PNT B, PNT C, float s) {return( P(P(B,s/4.,A),0.5,P(B,s/4.,C))); };                          // returns a tucked B towards its neighbors
PNT F(PNT A, PNT B, PNT C, PNT D, float s) {return( P(P(A,1.+(1.-s)/8.,B) ,0.5, P(D,1.+(1.-s)/8.,C))); };    // returns a bulged mid-edge point 
