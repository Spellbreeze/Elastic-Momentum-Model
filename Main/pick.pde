...
PNT PP=P(); // picked point
Boolean  picking=false, center=true;

float dz=-400;                                 // distance to camera. Manipulated with wheel or when 's' is pressed and mouse moved
//float dz=0;                                 // distance to camera. Manipulated with wheel or when 's' is pressed and mouse moved
float rx=-0.25*TWO_PI, ry=-0.00*TWO_PI;     // view angles manipulated when space bar (but not mouse) is pressed and mouse is moved
PNT Viewer = P(); // location of the viewpoint
PNT F = P(0,0,0);  // focus point:  the camera is looking at it (moved when 'f or 'F' are pressed
PNT Of=P(100,100,0), Ofp=P(100,100,0); // current and previous point on the floor under the mouse
boolean viewpoint=false;  // set to show frozen viewpoint and frustum from a different angle

//*********** TOOLS FOR 3D PICK ******************
PNT ToScreen(PNT P) {return P(screenX(P.x,P.y,P.z),screenY(P.x,P.y,P.z),0);}  // O+xI+yJ+kZ
PNT ToModel(PNT P) {return P(modelX(P.x,P.y,P.z),modelY(P.x,P.y,P.z),modelZ(P.x,P.y,P.z));}  // O+xI+yJ+kZ

VCT I=V(1,0,0), J=V(0,1,0), K=V(0,0,1); // screen projetions of global model frame

void computeProjectedVectors() { 
  PNT O = ToScreen(P(0,0,0));
  PNT A = ToScreen(P(1,0,0));
  PNT B = ToScreen(P(0,1,0));
  PNT C = ToScreen(P(0,0,1));
  I=V(O,A);
  J=V(O,B);
  K=V(O,C);
  }
...
public PNT pick(int mX, int mY) { // returns point on visible surface at pixel (mX,My)
  PGL pgl = beginPGL();
...
public PNT pick(float mX, float mY, float mZ) { 
  PGraphics3D p3d = (PGraphics3D)g;
  PMatrix3D proj = p3d.projection.get();
  PMatrix3D modelView = p3d.modelview.get();
  PMatrix3D modelViewProjInv = proj; modelViewProjInv.apply( modelView ); modelViewProjInv.invert();
  ...
  void doPick()
   ...
   if(mousePressed&&!keyPressed) // when mouse is pressed (but no key), show red ball at surface point under the mouse (and its shadow)
     {
     PickedFocus=false;  
     fill(magenta); show(Of,6);  fill(grey); show(Ofp,6); 
     }  
    // Locks focus on point Of until Of is reset (mouse pressed, but no key)
    if(PickedFocus) F=P(Of); 
    // show focus point F and its shadow
    fill(white); show(F,4); // magenta focus point (stays at center of screen)
    fill(grey,100); showShadow(F,3); // magenta translucent shadow of focus point (after moving it up with 'F'
...
PNT viewPoint() {return pick( 0,0, (height/2) / tan(PI/6));}
/*  
